  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button
          type="button"
          // ✅ ZMIANA: Usunięto AnimatePresence i isVisible (opóźnienie), zamieniono na zwykły button
          className="fixed bottom-32 right-4 lg:bottom-8 lg:right-8 z-50 group flex items-center justify-center cursor-pointer appearance-none outline-none"
          aria-label="Masz pomysł?"
        >
          {/* Dymek z tekstem (tylko desktop) */}
          <div className="hidden md:flex absolute right-full mr-4 bg-background border-2 border-foreground px-3 py-2 rounded-md whitespace-nowrap shadow-[4px_4px_0px_0px_var(--foreground)] items-center gap-2">
          ...
          {/* Główny przycisk */}
          <div className="relative bg-primary text-background border-2 border-foreground p-3 md:p-4 rounded-full shadow-[4px_4px_0px_0px_var(--foreground)] hover:shadow-[2px_2px_0px_0px_var(--foreground)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all duration-200 active:scale-95">
